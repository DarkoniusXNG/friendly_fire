if zuus_cloud_ff == nil then
	zuus_cloud_ff = class({})
end

LinkLuaModifier( "modifier_zuus_cloud_ff", "heroes/zeus/cloud.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_zuus_bolt_true_sight", "heroes/cloud.lua", LUA_MODIFIER_MOTION_NONE )

function zuus_cloud_ff:OnSpellStart()
  ...
  hCloud:AddNewModifier( caster, self, "modifier_zuus_cloud_ff", nil )
...
function zuus_cloud_ff:OnHeroCalculateStatBonus()
	local caster = self:GetCaster()
...
modifier_zuus_cloud_ff = class( ModifierBaseClass )

function modifier_zuus_cloud_ff:IsHidden()
  return true
end

function modifier_zuus_cloud_ff:IsDebuff()
  return false
end

function modifier_zuus_cloud_ff:IsPurgable()
  return false
end

function modifier_zuus_cloud_ff:RemoveOnDeath()
  return true
end

function modifier_zuus_cloud_ff:OnCreated( kv )
  if IsServer() then
...
function modifier_zuus_cloud_ff:OnDestroy()
  if IsServer() then
...
function modifier_zuus_cloud_ff:DeclareFunctions()
	local funcs =
	{
...
function modifier_zuus_cloud_ff:GetAbsoluteNoDamagePhysical()
  return 1
end

function modifier_zuus_cloud_ff:GetAbsoluteNoDamageMagical()
  return 1
end

function modifier_zuus_cloud_ff:GetAbsoluteNoDamagePure()
  return 1
end

function modifier_zuus_cloud_ff:OnAttacked(params)
...
function modifier_zuus_cloud_ff:OnIntervalThink()
...
function modifier_zuus_cloud_ff:CastLightningBolt(target)
  local caster = self:GetCaster()
  local parent = self:GetParent()
